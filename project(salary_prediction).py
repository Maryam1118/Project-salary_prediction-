...
st.markdown('<p class="big-title">💼 Salary Prediction Dashboard</p>', unsafe_allow_html=True)
...
with tab1:
    ...
    if submitted:
        ...
        st.success(f"Predicted Salary: ₹ {round(pred,2)}")
...
# =====================================================
with tab3:
    st.header("💰 Salary Distribution in Dataset")
    fig = plt.figure()
    ...
    st.header("📈 Relationship Between Features and Salary")
    for col in num_features:
        if col != "Salary":
